...
func Error(request *http.Request, response http.ResponseWriter, message string, err error, httpError *HttpError) {
    ...
    notify.Message(fmt.Sprintf("Error encountered.\n\nIP: %s\nMethod: %s\nURL: %s", request.RemoteAddr, request.Method, request.URL.Path))
...
func send(request *http.Request, response http.ResponseWriter, contentType string, body string, statusCode int) {
    if global.Debug {
        ...
        log.Printf("\nIP: %s\nMethod: %s\nURL: %s\nAuthorization: %s\nBody: %s\nResponse: %s\n", request.RemoteAddr, request.Method, request.URL.Path, request.Header.Get("authorization"), buffer, body)
